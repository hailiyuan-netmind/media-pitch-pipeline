...
1b. **放行必须是明确的放行词**。owner 对文案的修改建议、方案讨论、"可以这样写"类
   消息一律视为设计输入而非发送授权；只有"放行/发吧/可以发"或 M 列审核标记才构成
   放行。**任何 owner 未见过的新文本（哪怕一句话）严禁直接外发**——必须先展示再等指令
   （2026-07-21 事故实录：把披露方案建议误读为双发放行令，插入未过目的披露句即发送）。
...
2. **绝不编造引用**。夸赞开头引用的文章必须是你亲自打开读过的；核实不了就换一篇
   或如实标记，不许硬写。
2a. **逐句事实核查后才许发送**。开头与结尾句中每个事实性表述（平台、年限、数量、
   论点、例子）都必须能在来源上找到依据；收件人是最了解自己的人，一处不准确
   整封作废。全文对收件人第二人称。
